...
MODULE_PARM_DESC(search_enable_mode, "search  mode. 0: disable, 1: enable");
...
#endif
	pr_cont("bytes_acked_prev: %u, ", ca->bytes_acked_prev);
...
static inline void bictcp_search_normalize(struct sock *sk)
{
	...
	for(i = 0; i < n; i++) {
		if (acked_bytes < bytes_per_bin * 2) //last one.
...
static void hystart_search_update(struct sock *sk, u32 rtt_us)
{
	...
	if (now >= ca->epoch_expires_us) { 
#if 0 
		bictcp_search_normalize(sk);
#else
		//TODO decide not to use normalize 
		...
		ca->epoch_expires_us = now + ca->epoch_dur_us;
	}
...
static int __init cubictcp_register(void)
{
	...
	pr_info("TCP Cubic with SEARCH registered. ICSK_CA_PRIV_SIZE: %lu, sizeof(struct bictcp): %lu.\n", ICSK_CA_PRIV_SIZE, sizeof(struct bictcp));
